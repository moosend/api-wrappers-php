...
class MailingListTest extends \PHPUnit_Framework_TestCase {
	...
	public function test_Can_Create_MailingList_Instance() {
		...
		public function test_Can_Create_MailingList_Instance_When_Providing_Valid_Json_Data_To_Custom_Constructor() {
			$this->assertEquals('04fad8e2-2b35-4302-a887-58f14a1152ab', $this->_mailingList->ID);
			$this->assertEquals('Your List Name', $this->_mailingList->Name);
			$this->assertEquals(1024, $this->_mailingList->ActiveMemberCount);
			$this->assertEquals(16, $this->_mailingList->BouncedMemberCount);
			$this->assertEquals(32, $this->_mailingList->RemovedMemberCount);
			$this->assertEquals(24, $this->_mailingList->UnsubscribedMemberCount);
			$this->assertEquals(0, $this->_mailingList->Status);
			
			$this->assertEquals(CustomFieldDefinition::withJSON($this->_jsonData['CustomFieldsDefinition'][0])->ID, $this->_mailingList->CustomFieldsDefinition[0]->ID);
		
			$this->assertEquals('127.0.0.1', $this->_mailingList->CreatedBy);
			$this->assertEquals('/Date(1368710504000+0300)/', $this->_mailingList->CreatedOn);
			$this->assertEquals('127.0.0.1', $this->_mailingList->UpdatedBy);
			$this->assertEquals('/Date(1368710923000+0300)/', $this->_mailingList->UpdatedOn);
		}
}